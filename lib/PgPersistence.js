...
class PgPersistence {
  ...
  async getAllTodoLists(username) {
    ...
    const GET_TODOS = {
      text: "SELECT * FROM todos WHERE username = $1 AND todolist_id = $2",
      values: [username]
    };

    let dbResponse = await dbQuery(GET_LISTS);
    ...
    for (let idx in lists) {
      ...
      let todos = await dbQuery(GET_TODOS);
  ...
  async getTodoList(listID, username) {
    ...
    const GET_TODOS = {
      text: "SELECT * FROM todos WHERE todolist_id = $1 AND username = $2",
      values: [listID, username]
    };

    let response1 = await dbQuery(GET_LIST);
    let list = response1.rows[0];
    let response2 = await dbQuery(GET_TODOS);
  ...
  async duplicateTitleExists(prospectiveTitle, username) {
    ...
    let response = await dbQuery(GET_ALL_LIST_TITLES);
  ...
  async addTodoList(listTitle, username) {
    ...
    let response = await dbQuery(ADD_LIST);
  ...
  async destroyList(listID, username) {
    ...
    let response = await dbQuery(DELETE_LIST);
  ...
  async setListTitle(listID, newTitle, username) {
    ...
    let response = await dbQuery(UPDATE_LIST_TITLE);
  ...
  async addToDo(listID, todoTitle, username) {
    const ADD_TODO = {
      text: "INSERT INTO todos (todolist_id, title, username) VALUES ($1, $2, $3)",
      values: [listID, todoTitle, username]
    }

    let response = await dbQuery(ADD_TODO);
  ...
  async toggleTodoDone(listID, todoID, username) {
    const TOGGLE_DONE = {
      text: "UPDATE todos SET done = NOT done WHERE todolist_id = $1 AND id = $2 AND username = $3",
      values: [listID, todoID, username]
    }

    let result = await dbQuery(TOGGLE_DONE);
    return result.rowCount > 0;
  }

  async destroyTodo(listID, todoID, username) {
    const DELETE_TODO = {
      text: "DELETE FROM todos WHERE todolist_id = $1 AND id = $2 AND username = $3",
      values: [listID, todoID, username]
    }

    let response = await dbQuery(DELETE_TODO);
  ...
  async markAllDone(listID, username) {
    const MARK_ALL_TODOS_IN_LIST_DONE = {
      text: "UPDATE todos SET done = true WHERE todos.todolist_id = $1 AND username = $2",
      values: [listID, username]
    }

    let response = await dbQuery(MARK_ALL_TODOS_IN_LIST_DONE);
  ...
  async validateCredentials(username, password) {
    ...
    let response = await dbQuery(GET_UNAME_AND_PASSW);
  ...
  //   let response1 = await dbQuery(GET_ALL_PAIRS);
